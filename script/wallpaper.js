...
module.exports.config = {
  ...
  version: "1.0.0",
  aliases: ['wall'],
  description: "Search for images on PixaBay based on a query and fetch a specified number of images (1-100).",
  cooldown: 5,
};
  ...
  try {
    ...
    if (!keySearch.includes("-")) {
      return api.sendMessage(
        "Please enter your search query followed by a hyphen (`-`) and the number of images to fetch (e.g., `tree -5`).",
        event.threadID,
        event.messageID
      );
    }
    ...
    // Fetch image URLs
    const apiUrl = `https://aryanchauhanapi.onrender.com/api/wallpaper?query=${encodeURIComponent(keySearchs)}&number=${numberSearch}`;
    const res = await axios.get(apiUrl);
    const data = res.data.largeImageURL;
