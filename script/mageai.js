...
module.exports.run = async function({
  ...
  try {
    ...
    if (!prompt) {
      await api.sendMessage(`Invalid prompts.`, event.threadID, event.messageID);
    } else {
      const response = await axios.get(`https://aryanchauhanapi.onrender.com/api/mageai?prompt=${encodeURIComponent(prompt)}`, { responseType: 'stream' });
