...
module.exports.run = async function({
  ...
  try {
    ...
    if (!prompt) {
      await api.sendMessage(`Please provide a prompt for the image.`, event.threadID, event.messageID);
    } else {
      const response = await axios.get(`https://aryanchauhanapi.onrender.com/xl3?prompt=${encodeURIComponent(prompt)}`, { responseType: 'stream' });
