...
module.exports.run = async function({
  ...
  try {
    ...
    if (!prompt) {
      await api.sendMessage(`Please provide a prompt for the image generation. Example: \`generateimage cute cat\``, event.threadID, event.messageID);
    } else {
      const response = await axios.get(`https://aryanchauhanapi.onrender.com/v1/generate?prompt=${encodeURIComponent(prompt)}`);
